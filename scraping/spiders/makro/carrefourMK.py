...
# ajuste aqui conforme o DOM real do Carrefour
CSS_SELECT_NOME_PDP = """
h1.vtex-store-components-3-x-productName::text,
.vtex-store-components-3-x-productBrand::text,
.vtex-product-name-1-x-productName::text
"""

CSS_SELECT_PRECO_PDP = """
.vtex-product-price-1-x-sellingPriceValue::text,
.vtex-product-price-1-x-listPriceValue::text
"""

CSS_SELECT_NOME_LISTAGEM = """
.vtex-product-summary-2-x-productName::text,
.vtex-product-summary-2-x-productBrandName::text
"""

CSS_SELECT_PRECO_LISTAGEM = """
.vtex-product-price-1-x-sellingPriceValue::text,
.vtex-product-price-1-x-listPriceValue::text
"""
...
class CarrefourMKSpider(Spider):
    ...
    def _ler_eans_xlsx(self, caminho: Path):
        ...
        # se não encontrou, usa primeira coluna não vazia
        if idx_ean is None:
    ...
    def montar_url_busca(self, termo: str) -> str:
        ...
        return f"https://www.carrefour.com.ar/{termo}?_q={termo}&map=ft"
    ...
    def extrair_preco_linha(self, texto: str):
        if not texto:
            return None
    ...
    def extrair_nome_listagem(self, card):
        textos = card.css(CSS_SELECT_NOME_LISTAGEM).getall()
    ...
    def parse_search(self, response):
        ...
        # 1) tentar achar links de produto na listagem (/p)
        ...
        # 2) fallback: tentar extrair algo da própria listagem
        cards = sel.css("article, section, div")
        encontrou_algo = False

        for card in cards:
            html_card = card.get() or ""
            if "Mínimo de compra" in html_card:
                continue
            if "$" not in html_card:
                continue

            preco_textos = card.css(CSS_SELECT_PRECO_LISTAGEM).getall()
            preco_textos = [
                " ".join(t.split()) for t in preco_textos if t and t.strip()
            ]
            preco = None
            for t in preco_textos:
                preco = self.extrair_preco_linha(t)
                if preco:
                    break
    ...
    def parse_produto(self, response):
        ...
        textos_nome = sel.css(CSS_SELECT_NOME_PDP).getall()
        textos_nome = [" ".join(t.split()) for t in textos_nome if t and t.strip()]
        nome = textos_nome[0] if textos_nome else None

        preco_textos = sel.css(CSS_SELECT_PRECO_PDP).getall()
        preco_textos = [" ".join(t.split()) for t in preco_textos if t and t.strip()]
        preco = None
        for t in preco_textos:
            preco = self.extrair_preco_linha(t)
            if preco:
